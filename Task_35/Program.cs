...
int FindProperElemInArray (int[] arr, int minNum, int MaxNum)
{
    int totalNumbers=0;
    for (int i = 0; i < arr.Length; i++)
    {
        if ((arr[i]>=minNum)&&(arr[i]<=MaxNum) ) totalNumbers++;
    }
    return totalNumbers;
}
...
PrintArray(array);
Console.WriteLine();
int result = FindProperElemInArray(array,10,99);
Console.Write($"количество элементов на отрезке [10,99]: {result}");
